-- ============================================================================
-- finalize_attempt_with_results: also return is_within_window
-- ----------------------------------------------------------------------------
-- The client used to read `is_within_window` with a separate SELECT against
-- the `attempts` table immediately after calling this RPC. That extra round
-- trip is (a) wasted work, and (b) prone to read-after-write races against
-- replicas. Returning the field in the same RPC fixes both.
--
-- The flag itself is already maintained by triggers and existing finalize
-- logic — we only widen the RETURNS TABLE.
-- ============================================================================
...
BEGIN
  ...
  -- Idempotent path: attempt already finalized (e.g. retry from reprocessing
  -- queue). Return the persisted snapshot without re-counting.
  ...
  -- Re-read so we get the (trigger-updated) is_within_window value.
