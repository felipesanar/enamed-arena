...
--
-- O corpo abaixo é cópia do corpo capturado em produção via pg_get_functiondef()
-- (ver migrations-log.md), com uma única troca: `auth.uid()` -> `p_user_id`.
-- A mensagem de erro 'Attempt not found for current user' é mantida literal
-- (produção diverge do texto do brief original, que dizia 'for user'; a wrapper
-- delega para esta função, então qualquer caller que hoje casa essa string
-- continua vendo o mesmo texto).
...
-- Passa a delegar para a variante explícita, resolvendo o usuário via auth.uid()
-- exatamente como o corpo original fazia internamente. Nenhuma lógica de score
-- é duplicada aqui.
