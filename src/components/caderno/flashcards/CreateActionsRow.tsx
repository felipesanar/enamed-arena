...
export function CreateActionsRow({ onGenerate, onCreate, createDisabled }: CreateActionsRowProps) {
  const createButton = (
    <button
      ...
      className={cn(
        'group flex h-full w-full flex-1 items-center gap-3 rounded-[var(--c-radius-card)] border p-4 text-left',
    ...
    >
      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--c-soft-wine-bg)]">
        <Plus className="h-5 w-5 text-[var(--c-wine-600)] dark:text-[var(--c-wine-300)]" aria-hidden />
      </span>
    ...
    <div className="flex flex-col gap-2 sm:flex-row sm:gap-3">
      ...
      <button
        ...
        className={cn(
          ...
          'border-amber-400/40 bg-gradient-to-r from-amber-50 via-[var(--c-surface)] to-[var(--c-surface)] dark:from-amber-500/[0.1] dark:via-[var(--c-surface)] dark:to-[var(--c-surface)]',
          'transition-all duration-150',
      ...
      {createDisabled ? (
        <Tooltip delayDuration={300}>
          <TooltipTrigger asChild>
            <span className="block h-full flex-1 cursor-not-allowed" tabIndex={0}>{createButton}</span>
          </TooltipTrigger>
