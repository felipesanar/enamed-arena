...
import { computePerformanceBreakdown, type ThemePerformance } from '@/lib/result-helpers';
...
/** Decomposition tree node — adapted from Academy's pattern */
    ...
    >
      <div className="flex justify-between items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          <span className={cn('text-caption', isSelected ? 'text-primary-foreground/70' : 'text-muted-foreground')}>
            {correct}/{total}
          </span>
          <span className={cn('font-bold text-body-sm tabular-nums', isSelected ? 'text-primary-foreground' : 'text-primary')}>
            {score}%
          </span>
        </div>
      </div>
...
export default function DesempenhoPage() {
  ...
  // Compute performance for selected simulado
  const selectedSimulado = simuladosWithResults.find(s => s.id === selectedSimuladoId);
  ...
  return (
    <AppLayout>
      ...
      {simuladosWithResults.length > 1 && (
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {simuladosWithResults.map(s => (
            <button
              key={s.id}
              onClick={() => { setSelectedSimuladoId(s.id); setSelectedArea(null); }}
              className={cn(
      ...
      {/* Performance Summary — Academy pattern */}
      {byArea.length > 1 && (
        <PremiumCard className="p-5 md:p-6 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="rounded-xl border border-success/20 bg-success/[0.03] p-4">
              <h4 className="font-semibold flex items-center gap-2 text-success text-body-sm mb-2">
                <Star className="h-4 w-4" /> Ponto Forte
              </h4>
              <p className="text-body-sm text-muted-foreground">
                Sua principal fortaleza foi em <strong className="text-foreground">{bestArea.area}</strong> com <strong className="text-foreground">{bestArea.score}%</strong> de acertos.
              </p>
            </div>
            <div className="rounded-xl border border-destructive/20 bg-destructive/[0.03] p-4">
              <h4 className="font-semibold flex items-center gap-2 text-destructive text-body-sm mb-2">
                <TrendingDown className="h-4 w-4" /> Oportunidade de Melhoria
              </h4>
              <p className="text-body-sm text-muted-foreground">
                A área com maior oportunidade é <strong className="text-foreground">{worstArea.area}</strong> com <strong className="text-foreground">{worstArea.score}%</strong> de acertos.
              </p>
            </div>
          </div>
        </PremiumCard>
      ...
      {/* Hierarchical Analysis — Academy's DecompositionTree */}
      <PremiumCard className="p-5 md:p-6 mb-8">
        <div className="flex items-center gap-2.5 mb-4">
          <div className="h-8 w-8 rounded-xl bg-primary/10 flex items-center justify-center">
        ...
        <div className="flex flex-col lg:flex-row gap-5">
          ...
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-5">
                  ...
                  {byArea.map(area => (
                    ...
                    >
                      <PerformanceNode
                        ...
                        onClick={() => setSelectedArea(prev => prev === area.area ? null : area.area)}
                      />
                    </motion.div>
                  ))}
                </AnimatePresence>
            ...
            <div>
              ...
              {!selectedArea ? (
                <div className="flex items-center justify-center h-32 text-center text-muted-foreground/60 text-body-sm p-4 border border-dashed border-border/40 rounded-xl">
