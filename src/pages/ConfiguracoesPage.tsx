...
export default function ConfiguracoesPage() {
  ...
  console.log('[ConfiguracoesPage] Rendering, segment:', segment);
        ...
        <>
          <SectionHeader
            ...
            action={
              <Link
                to="/onboarding"
                className="inline-flex items-center gap-1.5 text-body-sm text-primary hover:text-wine-hover transition-colors font-medium"
              >
            ...
            <PremiumCard className="p-5" delay={0.12}>
              ...
              <div className="flex flex-wrap gap-1.5 mt-1">
                {onboarding.targetInstitutions.map(inst => (
                  <span key={inst} className="px-2 py-0.5 rounded bg-accent text-accent-foreground text-caption font-medium">
                    {inst}
                  </span>
                ))}
              </div>
