...
- `build`: installs full `devDependencies` and compiles TypeScript
  (`npm run build`, i.e. `tsc`) to produce `dist/`.
- `runtime`: the Tectonic + vendored-fonts + cached-LaTeX-packages image,
  ...
  `templates/` cross the stage boundary. A final smoke-test recompiles
  `test/fixtures/minimal.tex` with Tectonic *after* every Node-related
  layer, confirming the app image didn't disturb the Tectonic/fonts/
  packages setup.
